...
public class FilmService {
    ...
    public List<Genre> listarGenerosDoFilme(int filmID) throws Exception {
        ...
        for (FilmGenre relacao : relacoes) {
            if (relacao != null && relacao.getGenreID() > 0 && !idsAdicionados.contains(relacao.getGenreID())) {
                Genre genre = genreDao.read(relacao.getGenreID());

                if (genre != null) {
                    generos.add(genre);
                    idsAdicionados.add(relacao.getGenreID());
                }
            }
        }
    ...
    private void preencherGenerosDoFilme(Film film) throws Exception {
        ...
        if (relacoes == null || relacoes.isEmpty()) {
            film.setGenreIDs(new int[0]);
            film.setGeneros(new String[] { "-" });
            return;
        }
    ...
    private void preencherNomesGeneros(Film film) throws Exception {
        if (film == null || film.getGenreIDs() == null || film.getGenreIDs().length == 0) {
            if (film != null) {
                film.setGeneros(new String[] { "-" });
            }
            return;
        ...
        if (nomes.isEmpty()) {
            ...
            film.setGeneros(nomes.toArray(new String[0]));
        }
    }
