...
public class ReviewDAO {
    ...
    public int create(Review r) throws Exception {
        ...
        int recordSize = 1 + 4 + 4 + 4 + 1 + 2 + noteBytes.length;
    ...
    public boolean update(Review r) throws Exception {
        ...
        if (antiga != null) {
            if (antiga.getFilmID() != r.getFilmID()) {
                indiceReviewPorFilme.atualizar(antiga.getFilmID(), r.getFilmID(), r.getID());
            } else {
                indiceReviewPorFilme.inserir(r.getFilmID(), r.getID());
            }

            if (antiga.getUserID() != r.getUserID()) {
                indiceReviewPorUsuario.atualizar(antiga.getUserID(), r.getUserID() ,r.getID());
            } else {
