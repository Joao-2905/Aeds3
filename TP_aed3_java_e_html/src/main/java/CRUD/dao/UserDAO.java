...
import main.java.CRUD.index.Bucket;
import main.java.CRUD.index.HashExtensivel;
import main.java.CRUD.index.IndexEntry;
...
public class UserDAO {
    ...
    // VALIDAÇÃO PERMANENTE DO ÍNDICE
    public void validarIndice() throws IOException {
        ...
        for (int i = 0; i < indice.getDiretorio().size(); i++) {
            ...
            Bucket b = indice.getDiretorio().get(i);

            for (IndexEntry e : b.getEntradas()) {

                int id = e.getId();
                long pos = e.getEndereco();
